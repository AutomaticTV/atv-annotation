...
Build for ATV Deployment (Windows + Anaconda)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Reminder. We use pyinstaller to compile the Annotation Tool. Pyinstaller supports multiple OS but it is not a cross-platform compiler, i.e. to compile an executable for Windows, it needs to be compiled from Windows.

#.  Get (git clone) the Pyinstaller repository from https://github.com/pyinstaller/pyinstaller.git.
#.  Set up Anaconda:

    #.  Download and install Anaconda Individual Edition from https://www.anaconda.com/products/individual.
        Warning: The installer will prompt with an option to add Anaconda to your Path variable. Do not choose this option if you have other versions of python or Anaconda already running in your system.
    #.  Open Anaconda Powershell Prompt.
    #.  Create an Anaconda environment with the dependencies via Anaconda Powershell Prompt.
        .. code::

            > conda env create -n <env_name> -f anaconda_requirements.yml
        
        Warning: packages "pymongo" and "bson" may cause problems. You can reinstall them by:
...
#.  Run build_windos.ps1 powershell script uisng the Anaconda Powershell Prompt with the createt environment activated. Use -CompressBuild option to build ZIP file ready for deployment.
#.  Deploy ATV-Annotation-<VERSION> ZIP file. Alternatively, deploy "dist" directory.
